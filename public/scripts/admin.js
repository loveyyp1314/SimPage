...
const DEFAULT_ADMIN_DATA = Object.freeze({
  passwordHash:
    "1a968cba0c9a05b2b235aa54a29bc91ef30a5a8a202dc290cf862070e14e259fad87c94f6f33ce1b2b36a75b233ef282b1298ca12fc96894a3abf38ff9e75b8a",
...
function normaliseStatsIncoming(raw) {
  const fallback = { ...DEFAULT_STATS };
  if (!raw || typeof raw !== "object") {
    return fallback;
  }
  const count = Number(raw.visitorCount);
  return {
    visitorCount: Number.isFinite(count) && count >= 0 ? Math.round(count) : fallback.visitorCount,
  };
}

function normaliseAdminIncoming(raw) {
  if (!raw || typeof raw !== "object") {
...
function updateStateFromResponse(data) {
  const normalisedApps = normaliseIncoming(data?.apps, "apps");
  const normalisedBookmarks = normaliseIncoming(data?.bookmarks, "bookmarks");
  const normalisedSettings = normaliseSettingsIncoming(data?.settings);
  const normalisedStats = normaliseStatsIncoming(data?.stats);
  const normalisedAdmin = normaliseAdminIncoming(data?.admin);

  state.apps = normalisedApps;
  state.bookmarks = normalisedBookmarks;
  state.settings = normalisedSettings;

  latestFullData = {
    apps: cloneItems(normalisedApps),
    bookmarks: cloneItems(normalisedBookmarks),
...
function updateModeBadge(text) {
  ...
  modeBadge.style.display = text ? "inline-block" : "none";
}
